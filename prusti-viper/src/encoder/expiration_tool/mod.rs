...
impl<'a, 'tcx: 'a> ExpirationTools<'tcx> {
    ...
    pub fn expire(self,
        ...
        if let Some(place) = places.next() {
            self.into_iter().flat_map(|et|
                if let Some(ets) = et.clone().expire(place) {
                    ets.0
                } else {
                    vec![et.clone()]
                }
...
impl<'a, 'tcx: 'a> ExpirationTool<'tcx> {
    ...
    fn magic_wand(&self,
        place: &'a places::Place<'tcx>
    ) -> Option<MagicWand<'tcx>> {
        self.magic_wands().cloned().filter(|mw| mw.expired() == place).next()
    }

    fn expire(self,
        place: &'a places::Place<'tcx>
    ) -> Option<ExpirationTools<'tcx>> {
