...
/// This represents the assignment in which a loan was created. The `source` will contain the creation of the loan, while the `dest` will store the created reference.
#[derive(Clone, Debug)]
    ...
    for (point, regions) in outlives_at_point {
            ...
            .all(|(_, _, loan_point)| *loan_point != point)
        {
            ...
            if is_call(&mir, location) {
                ...
                // TODO: This is used later to determine which loans are zombies. In the assignment
                //    _1 = move _0;
                //  assuming _0 stores a loan L0 and _1 stores the new fake loan L1, what is the
                //  zombie? I would have thought L0 becomes a zombie because the move replaced it
                //  by L1. But this line adds the newly created loan to reference_moves, which
                //  means L1 will be considered a zombie later.
