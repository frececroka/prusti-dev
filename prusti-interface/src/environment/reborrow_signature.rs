...
use crate::environment::mir_utils::AsPlace;
use crate::environment::mir_utils::enumerate_all_places;
use crate::environment::mir_utils::PlaceAddProjection;
...
impl<'tcx> ReborrowSignature<mir::Place<'tcx>> {
    ...
    pub fn construct(
        ...
        let return_place = mir::RETURN_PLACE.as_place();
...
impl<'a, P: Eq + Hash + fmt::Debug + 'a> ReborrowSignature<P> {
    ...
    /// Returns the set of all input references that are not blocked by any output reference.
    pub fn returned_inputs(&self) -> HashSet<&P> {
        self.inputs.difference(&self.blocked).collect()
    }
}
...
fn gather_outlives(param_env: ParamEnv) -> HashMap<Region, HashSet<Region>> {
    ...
    let outlives = param_env.caller_bounds().iter()
        .filter_map(|b| match b.kind() {
            PredicateKind::RegionOutlives(p) => Some(p),
            _ => None
        })
        .map(|o| o.skip_binder())
